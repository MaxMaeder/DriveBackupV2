...
public class CommandHandler implements CommandExecutor {
    ...
    private Component genCommandHelpMessage(String command, String description) {
        return Component.text()
                ...
                .append(Component.text(" - " + description, NamedTextColor.DARK_AQUA)).build();
    }
    ...
    private void sendHelpResources(CommandSender player) {
        player.sendMessage(
                ChatColor.GOLD + "|======" + ChatColor.DARK_RED + "DriveBackupV2" + ChatColor.GOLD + "======|");
        player.sendMessage(ChatColor.DARK_AQUA + "Need help? Check out these helpful resources!");
        DriveBackup.adventure.sender(player).sendMessage(
            Component.text()
                .append(Component.text("Wiki: ", NamedTextColor.DARK_AQUA))
                .append(Component.text("http://bit.ly/3dDdmwK", NamedTextColor.GOLD)
                    .hoverEvent(HoverEvent.showText(Component.text("Go to URL")))
                    .clickEvent(ClickEvent.openUrl("http://bit.ly/3dDdmwK")))
                .build());
        DriveBackup.adventure.sender(player).sendMessage(
            Component.text().append(Component.text("Discord: ", NamedTextColor.DARK_AQUA))
                .append(Component.text("http://bit.ly/3f4VuuT", NamedTextColor.GOLD)
                    .hoverEvent(HoverEvent.showText(Component.text("Go to URL")))
    ...
    private void sendVersion(CommandSender player) {
        player.sendMessage(
                ChatColor.GOLD + "|======" + ChatColor.DARK_RED + "DriveBackupV2" + ChatColor.GOLD + "======|");
        player.sendMessage(
                ChatColor.DARK_AQUA + "Plugin version: " + ChatColor.GOLD + plugin.getDescription().getVersion());
        player.sendMessage(
                ChatColor.DARK_AQUA + "Java version: " + ChatColor.GOLD + System.getProperty("java.version"));
