...
public final class MySQLUploader {
    ...
    private void getDataInsertStatement(OutputStreamWriter sql, String name) throws SQLException, IOException {
        ...
        try (ResultSet rs = stmt.executeQuery(query)) {
            ...
            sql.append("\n--\n").append(SQL_START_PATTERN).append(" table insert : ").append(name).append("\n--\n");
     ...
     * @throws SQLException exception
     */
    private void getInsertStatements(@NotNull OutputStreamWriter sql, String name, Collection<String> blacklist) throws SQLException, IOException {
        sql.append("--");
        sql.append("\n-- Generated by DriveBackupV2");
        sql.append("\n-- https://dev.bukkit.org/projects/drivebackupv2");
        sql.append("\n-- Date: ").append(new SimpleDateFormat("h:mm M/d/yyyy").format(Instant.now()));
        sql.append("\n--");
